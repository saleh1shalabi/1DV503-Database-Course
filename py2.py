...
def checking_db(db1, DB):
    # function that checks if there is a database with the given name
    ...
    try:
        ...
        if x == 0:
            ...
            # when all create and inserting is done
            ...
            db.commit() # commiting the changes so all should be saved
...
def create_tables(db1 ,filen):
    # function that create tables in database of the given csv files
...
def add_prim_key(db1, table):
    ...
    ## this function will make the ID attribut as primary key in all tabels
    # but when its the table "memInAct" it will add all attributs as primary key
    pathmembINact = "memInAct"
...
def add_foreign(db1,table):
    # this function will add foreign key for the tables that needs it 
    # and its hardcoded  
...
def run(db1, menu):
    ...
    while choose != "0":
        ...
        if choose == "1": 
            ...
            try:       # check if the view allrady have been created else pass
